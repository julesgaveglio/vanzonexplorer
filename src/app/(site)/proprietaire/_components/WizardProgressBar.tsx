...
/* ─── Step Icons — Vanzon blue (var(--accent)) ─────────────────────────────── */

function IconUser({ active, done }: { active: boolean; done: boolean }) {
  if (done) return <CheckIcon />;
  return (
    <svg className={`w-5 h-5 ${active ? "text-[var(--accent)]" : "text-slate-400"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.8">
          ...
          <div key={step.label} className="flex-1 flex flex-col items-center relative">
            {i > 0 && (
              <div
                className={`absolute top-5 -left-1/2 w-full h-0.5 transition-colors duration-300 ${
                  isDone ? "bg-[var(--accent)]" : "bg-slate-200"
                }`}
              />
            )}
            <div
              className={`relative z-10 w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 ${
                isDone
                  ...
                  : isActive
                    ? "bg-blue-50 ring-2 ring-[var(--accent)]"
                    : "bg-slate-100"
              }`}
            >
            ...
            <span
              className={`mt-2 text-xs font-medium transition-colors duration-300 ${
                isActive ? "text-[var(--accent)]" : isDone ? "text-[var(--accent)]" : "text-slate-400"
              }`}
            >
              {step.label}
            </span>
