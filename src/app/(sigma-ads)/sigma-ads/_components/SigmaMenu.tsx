...
import {
  Menu, X,
  LayoutDashboard, MousePointerClick, Users,
  Play, FileText, Mail, LogOut,
} from "lucide-react";

const NAV_LINKS = [
  { href: "/sigma-ads", label: "Dashboard", icon: LayoutDashboard, exact: true },
  { href: "/sigma-ads/optin", label: "Opt-in", icon: MousePointerClick },
  { href: "/sigma-ads/vsl", label: "VSL", icon: Play, exact: true },
  { href: "/sigma-ads/vsl/transcript", label: "Transcript", icon: FileText },
  { href: "/sigma-ads/leads", label: "Leads", icon: Users },
  { href: "/sigma-ads/emails", label: "Emails", icon: Mail },
];

export default function SigmaMenu({ email }: { email: string }) {
  ...
  async function handleLogout() {
    await fetch("/api/sigma/auth", { method: "DELETE" });
  ...
  return (
    <>
      ...
      {open && (
        <div
          className="fixed inset-0 z-[70] bg-black/20 transition-opacity"
          ...
          <div className="px-6 py-4 border-b border-slate-100">
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="flex-1 bg-slate-50 border border-slate-200 text-sm font-medium text-slate-800 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#B9945F]/30 cursor-pointer"
              >
